(function(){"use strict";const t="mh_exit_shown";let a=Date.now(),n=!1;if(["/prihlaseni","/onboarding","/404","/profil"].some(e=>window.location.pathname.includes(e))||sessionStorage.getItem(t))return;function d(){const e=document.createElement("div");e.id="exit-intent-modal",e.style.cssText=`
        ...
        `,document.body.appendChild(e);function o(){e.style.animation="fadeOut 0.2s ease forwards",setTimeout(()=>e.remove(),200)}e.addEventListener("click",l=>{l.target===e&&o()}),document.getElementById("exit-close").addEventListener("click",o),document.getElementById("exit-dismiss").addEventListener("click",o),document.getElementById("exit-cta").addEventListener("click",()=>{sessionStorage.setItem(t,"1")})}function i(){n||Date.now()-a<15e3||typeof window.Auth<"u"&&window.Auth.isPremium?.()||(n=!0,sessionStorage.setItem(t,"1"),d())}document.addEventListener("mouseleave",e=>{e.clientY<=0&&i()});let r;document.addEventListener("visibilitychange",()=>{document.hidden?r=setTimeout(i,1e3):clearTimeout(r)});const s=document.createElement("style");s.textContent=`
